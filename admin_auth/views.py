...
# db.child("superusers").child("Sundarlala").set({"First Name":"Sundarlala","password":"sundar4341r"})
...
def upload_pdf(request):
    if request.method == 'POST':
        ...
        if not pdf_file or not selected_class:
            return render(request, 'upload.html', {'message': 'Please select a class and upload a file.'})
        ...
        if selected_class not in valid_classes:
            return render(request, 'upload.html', {'message': 'Invalid class selected.'})
