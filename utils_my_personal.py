...
class Firebase_Operations:
    ...
    def create_user_actual(self,firstname, lastname, email, password,phone,age,guardianname,classs,board):
        try:
            ...
            self.db.child("users").child(user_id).set({"First Name": firstname,"Last Name": lastname,"email": email,"Phone Number":phone,"Age":age,"Class":classs,"Guardian Name":guardianname,"Board":board})
            return user_id
            ...
            if s_users.each():
                for user in s_users.each():
                    if user.val().get("email") == email:
                        return False
            _users = self.db.child("signup_users").get()
            if _users.each():
